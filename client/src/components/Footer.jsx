...
function SocialIcon({ icon, s = 15, c = "currentColor" }) {
    ...
    <footer style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
          <span style={{ fontSize: 12, color: C.muted }}>{t('footer.language')}:</span>
          <LanguageSelector dark={dark} C={C} />
        </div>
      </div>
      ...
      <div style={{
        ...
        {SOCIAL_LINKS.map(s => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            style={{ color: C.muted, transition: "color 0.2s", display: "flex" }}
            onMouseEnter={e => e.currentTarget.style.color = s.color}
            onMouseLeave={e => e.currentTarget.style.color = C.muted}
          >
            <SocialIcon icon={s.icon} s={15} c="inherit" />
          </a>
        ))}
      </div>
